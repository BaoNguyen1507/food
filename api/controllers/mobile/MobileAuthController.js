...
const MobileAuthController = {
    ...
    getToken: async function (req, res) {
        ...
        if (!secret) return res.badRequest(AuthError.SYSTEM_SECRET_NOT_VALID);
        if (!codeVerify) return res.badRequest(AuthError.SYSTEM_CODE_VERIFY_NOT_VALID);
        ...
        if (myCodeVerify === codeVerify) {
            ...
            return res.badRequest(AuthError.SYSTEM_CODE_VERIFY_NOT_VALID);
        }
    },
    sampleToken: async function (req, res) {
        ...
        if (!codeVerify) return res.badRequest(AuthError.SYSTEM_TOKEN_REQUIRE);
    ...
    updateToken: async function (req, res) {
        ...
        if (!params.token) return res.badRequest(AuthError.SYSTEM_TOKEN_REQUIRE);

        //ALWAYS CHECK  OBJECT EXIST BEFORE UPDATE
        let authObj = AuthService.get({ id: params.token });
        if (!authObj) return res.notFound(AuthError.ERR_NOT_FOUND);
        //UPDATE DATA
