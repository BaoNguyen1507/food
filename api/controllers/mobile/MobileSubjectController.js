 ...
 */
const SubjectError = require('../../../config/errors/subject');
...
module.exports = {
  ...
  get: async (req, res) => {
    ...
    // CHECK ID PARAM
    if (!params.id) {
      return res.badRequest(SubjectError.ERR_ID_REQUIRED);
    }
    ...
    if (!subjects) {
      return res.notFound(SubjectError.ERR_NOT_FOUND);
    }
  ...
  trash: async (req, res) => {
    ...
    if (typeof params.ids === 'string') {
      if (!subjects.length) {
        return res.badRequest(SubjectError.ERR_NOT_FOUND);
      } else {
        // nothing to do
      }
    } else {
  ...
  search: async (req, res) => {
    ...
    const subjects = await SubjectService.find(where, bodyParams.limit, bodyParams.offset, bodyParams.sort),
      ...
      publish = await SubjectService.count({ status: sails.config.custom.STATUS.PUBLISH });
