 ...
 */
const TaxonomyError = require('../../../config/errors/taxonomy');
...
module.exports = {
  add: async (req, res) => {
    ...
    if (!params.title || !params.title.trim().length) {
      return res.badRequest(TaxonomyError.ERR_TITLE_REQUIRED);
    } else if (!params.alias || !params.alias.trim().length) {
      return res.badRequest(TaxonomyError.ERR_ALIAS_REQUIRED);
    }
  ...
  get: async (req, res) => {
    ...
    // CHECK ID PARAM
    if (!params.id) {
      return res.badRequest(TaxonomyError.ERR_ID_REQUIRED);
    }
    ...
    if (!taxonomy) {
      return res.notFound(TaxonomyError.ERR_NOT_FOUND);
    }
  ...
  trash: async (req, res) => {
    ...
    if (typeof params.ids === 'string') {
      if (!taxonomies.length) {
        return res.badRequest(TaxonomyError.ERR_NOT_FOUND);
      } else {
        // nothing to do
      }
    } else {
  ...
  search: async (req, res) => {
    ...
    const Taxonomies = await TaxonomyService.find(where, bodyParams.limit, bodyParams.offset, bodyParams.sort),
      ...
      publish = await TaxonomyService.count({ status: sails.config.custom.STATUS.PUBLISH });
